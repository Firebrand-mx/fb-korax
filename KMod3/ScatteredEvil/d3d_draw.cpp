	...
	else
	{
		...
		if(vtxStackPos == 3)
		{
			...
			if(FAILED(hr = IDirect3DDevice3_DrawPrimitive(d3dDevice,
				...
				I_Error("uploadVtx (DrawPrimitive) failed.\n");
			// Clear the stack.
			vtxStackPos = 0;
		}
		else
...
int	BeginScene(void)
{
	...
	{
		ST_Message( "drD3D.BeginScene: Failure! Error: %x\n", hr);
		return DGL_ERROR;
	}
	return DGL_OK;
	unguard;
...
int	EndScene(void)
{
	...
	{
		ST_Message( "drD3D.EndScene: Failure! Error: %x\n", hr);
		return DGL_ERROR;
	}
	return DGL_OK;
	unguard;
...
void Begin(int mode)
{
	...
	if(FAILED(hr = IDirect3DDevice3_Begin(d3dDevice, prim, VERTEX_FORMAT, 
		...
		ST_Message( "drD3D.Begin: Failure! Error: %x\n", hr);
	unguard;
}
...
void End(void)
{
	...
	if(primType != DGL_QUADS)
	{
		if(FAILED(hr = IDirect3DDevice3_End(d3dDevice, 0)))
			ST_Message( "drD3D.End: Failure! Error: %x\n", hr);
	}
