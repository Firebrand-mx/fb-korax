...
boolean EV_LineSearchForPuzzleItem(line_t *line, byte *args, mobj_t *mo)
{
	...
	for (i=0; i<player->artifactCount; i++)
	{
		...
		type = (artitype_t)(arti - arti_firstpuzzitem);
		if (type < 0) continue;
		if (type == line->arg1)
		{
...
static int shops[8][5][NUMARTIFACTS]=
{
	// Shop 1
	{{0,50,2,20,1,30,1,25,18,4,3,40,15,20,5,20},
	{0,50,2,20,1,30,1,25,18,4,3,40,15,20,5,20},
	{0,50,2,20,1,30,1,25,18,4,3,40,15,20,5,20},
	{0,50,2,20,1,30,1,25,18,4,3,40,15,20,5,20},
	{0,50,2,20,1,30,1,25,18,4,3,40,15,20,5,20}},
	// Shop 2
	{{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10},
	{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10},
	{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10},
	{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10},
	{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10}},
	// Shop 3
	{{0,50,2,10,1,30,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,30,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,30,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,30,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,30,1,13,18,2,3,20,8,10,5,10}},
	// Shop 4
	{{0,25,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,25,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,25,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,25,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,25,2,10,1,15,1,13,18,2,3,20,8,10,5,10}},
	...
	{0,50,2,20,1,30,1,25,18,2,3,20,8,20,5,10}},
	// Shop 7
	{{0,50,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,15,1,13,18,2,3,20,8,10,5,10},
	{0,50,2,10,1,15,1,13,18,2,3,20,8,10,5,10}},
...
//   if ()
   {
      ...
      player->money+=shops[args[0]][player->pclass][player->inventory[inv_ptr].type];
	  P_SetMessage(player, "SOLD ITEM",true);
...
boolean EV_BuySomething(byte *args, mobj_t *mo)
{
	player_t *player;
	boolean res;
	if (!mo) return false;
	...
	case PCLASS_MAGE:
		{
			if (args[0] == arti_invulnerability
				|| args[0] == arti_teleportother
				|| args[0] == arti_boostarmor)
			{
				P_SetMessage(player,"THE MAGE CAN'T USE THAT",true);
				return true;
			}
			break;
		}
	case PCLASS_CLERIC:
		{
			if (args[0] == arti_boostarmor)
			{
				P_SetMessage(player,"THE CLERIC CAN'T USE THAT",true);
				return true;
			}
			break;
		}
	case PCLASS_FIGHTER:
		{
			if (args[0] == arti_teleportother)
			{
				P_SetMessage(player,"THE FIGHTER CAN'T USE THAT",true);
				return true;
			}
			break;
		}
	...
	if (player->money<(unsigned int)(args[2]+(args[1]*10)))
	{
		P_SetMessage(player,"NOT ENOUGH MONEY",true);
		return true;
	}
	res = P_GiveArtifact(player,(artitype_t)args[0],NULL);
	if (res) {
		player->money-=(args[2]+(args[1])); //Remi: Removed *10
		P_SetMessage(player,"ARTIFACT BOUGHT",true);
		return true;
	} else {
		P_SetMessage(player,"25 IS THE MAX",true);
		return false;
	}
}
...
boolean EV_GivePoints(byte *args, mobj_t *mo)
{
	...
	{
	case 0:
		Give_Level(player,args[1]);
		break;
	case 1:
		player->maxhealth=args[1];
		break;
	case 2:
		player->speed+=args[2];
		break;
	case 3:
		player->strength+=args[3];
		break;
	case 4:
		player->agility+=args[4];
		break;
	case 5:
		Give_Experience(player,args[1]*256);
		break;
	case 6:
		player->intelligence+=args[5];
		break;
	case 7:
		player->wisdom+=args[6];
		break;
	default:
		break;
	}
	...
	mobj_t *mo)
{
	...
	switch(special)
	{
		...
		case 74: // Teleport_NewMap
			...
			{ // Only teleport when crossing the front side of a line
				if(!(mo && mo->player && mo->player->playerstate
					...
					&& (mo && mo->player 
					&& mo->player->pclass <= PCLASS_PIG)) //Possessed monsters can't exit maps
				{
		...
		case 145:
			buttonSuccess = EV_GivePoints(args, mo);
			break;
