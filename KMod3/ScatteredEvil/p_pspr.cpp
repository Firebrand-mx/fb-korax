...
{ // Mage - Ice wand - WP_MAGE_BWAND
	MANA_1,12,
	S_ICEWANDUP,S_ICEWANDDOWN,S_ICEWANDREADY,
	S_ICEWANDATK1_1,S_ICEWANDATK1_1,S_NULL,
	PCLASS_MAGE,1,
	5*FRACUNIT
},
...
{ // Fighter - Hammer2 - WP_FIGHTER_HAMMER2
	MANA_2,3,
	S_FHAM2UP,S_FHAM2DOWN,S_FHAM2READY_1,
	S_FHAM2ATK_1,S_FHAM2ATK_1,S_NULL,
	PCLASS_FIGHTER,2,
	10*FRACUNIT
},
...
void P_SetPsprite(player_t *player, int position, int stnum)
{
	...
	do
	{
		...
		if(state->action)
		{ // Call action routine.
			((state_action_t)state->action)(player, psp);
			if(!psp->state)
			{
				break;
			}
...
void P_SetPspriteNF(player_t *player, int position, int stnum)
{
	pspdef_t *psp;
	state_t *state;
...
int P_GetWeaponNum(player_t *player, newweapontype_t weapontype)
{
	...
	{		
		if (NewWeaponInfo[i].classtype == player->pclass)
		{
			if (weaponnumber == 50)
				weaponnumber = 0;
			else weaponnumber++;
	...
	{ //Special case, go for the best weapon possible and check mana
		...
		for (i = NUMACTUALWEAPONS-1; i > 0; i--)
		{
			...
			{ //Found possible weapon
				if ((pclass >= PCLASS_ETTIN || player->weaponowned[weaponnumber])
				...
				{ //Player has the weapon and don't have it up, check for mana
					...
					int manaCount = NewWeaponInfo[i].ammouse*10/player->wisdom;
...
//	statenum_t newstate;
	...
	newstate = NewWeaponInfo[player->pendingweapon].upstate;
...
boolean P_CheckMana(player_t *player)
{
	...
	count = NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom;
	if(mana == MANA_BOTH)
	{
...
//	statenum_t attackState;
	...
	//which needs it...
	if (player->pclass == PCLASS_CENTAURL && player->readyweapon == WP_CENTAURL_MISSILE)
		P_SetMobjState(player->mo, S_POSS_CENTAUR_MISSILE1);
	else
		P_SetMobjState(player->mo, PStateAttack[player->pclass]); // S_PLAY_ATK1);
...
void A_WeaponReady(player_t *player, pspdef_t *psp)
{
	...
	if (player->mo->reactiontime <20 && 
		(player->pclass == PCLASS_CENTAUR || player->pclass == PCLASS_CENTAURL) &&
		(player->readyweapon == WP_CENTAUR_SHIELD || player->readyweapon == WP_CENTAURL_SHIELD))
		player->mo->reactiontime = 2000;
...
hammerdone:
	if(player->mana[MANA_2] < 
		(int)(NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom))
...
void A_FHammerThrow(player_t *player, pspdef_t *psp)
{
	...
	player->mana[MANA_2] -= (NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom);
	mo = P_SpawnPlayerMissile(player->mo, MT_HAMMER_MISSILE); 
	if(mo)
	{
		mo->special1 = 0;
		mo->damage = mo->damage*player->intelligence/10;
	}	
}
...
void A_FSwordAttack(player_t *player, pspdef_t *psp)
{
	...
	player->mana[MANA_1] -= (NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom);
	player->mana[MANA_2] -= (NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom);
	pmo = player->mo;
...
void A_MWandAttack(player_t *player, pspdef_t *psp)
{
	...
	case WP_MAGE_WAND:
		...
		break;
	case WP_MAGE_WAND2:
		mo = P_SpawnPlayerMissile(player->mo, MT_MWAND2_MISSILE);
		break;
	case WP_MAGE_WAND3:
		mo = P_SpawnPlayerMissile(player->mo, MT_MWAND3_MISSILE);
		break;
	case WP_MAGE_WAND4:
		mo = P_SpawnPlayerMissile(player->mo, MT_MWAND4_MISSILE);
		break;
	}
	if(mo)
	{
		mo->thinker.function = (think_t)P_BlasterMobjThinker;
		mo->damage = mo->damage*player->intelligence/10;
	}
...
void A_MStaffAttack(player_t *player, pspdef_t *psp)
{
	...
	if (pmo)
	{
		pmo->damage = pmo->damage*player->intelligence/10;
	}
...
axedone:
	...
	{
		player->mana[MANA_1] -= 
			(NewWeaponInfo[player->readyweapon].ammouse*10/player->wisdom);		
	}
	return;		
}
...
void A_CMaceAttack(player_t *player, pspdef_t *psp)
{
	...
	switch(player->pclass) //This void is used by some possessed monsters, so damage needs change
	{
	case PCLASS_ETTIN:
		damage = HITDICE(2);
		PuffType = MT_HAMMERPUFF;
...
//			player->mo->angle = R_PointToAngle2(player->mo->x,
//				player->mo->y, linetarget->x, linetarget->y);
			if((player->pclass == PCLASS_REIVER) &&
				(linetarget->player || linetarget->flags&MF_COUNTKILL)
				&& (!(linetarget->flags2&(MF2_DORMANT+MF2_INVULNERABLE))))
			{
				newLife = player->health+(damage);
				newLife = newLife > mobjinfo[player->mo->type].spawnhealth
					? mobjinfo[player->mo->type].spawnhealth : newLife;
				player->mo->health = player->health = newLife;
			}
			goto macedone;
		}
		angle = player->mo->angle-i*(ANG45/16);
...
void A_CStaffCheck(player_t *player, pspdef_t *psp)
{
	...
	pmo = player->mo;
	damage = (20+(P_Random()&15))*player->intelligence/10;
...
void A_FireConePL1(player_t *player, pspdef_t *psp)
{
	...
	damage = 90+(P_Random()&15)*player->intelligence/10;
	for(i = 0; i < 16; i++)
	{
		angle = pmo->angle+i*(ANG45/16);
		slope = P_AimLineAttack(pmo, angle, MELEERANGE);
...
void P_SetupPsprites(player_t *player)
{
	...
	if (NewWeaponInfo[player->readyweapon].classtype != player->pclass)
...
void A_PSetReflective(player_t *player, pspdef_t *psp)
{
	...
	P_SetMobjState(player->mo, S_POSS_CENTAUR_BLOCK);
	A_SetInvulnerable(player->mo);
	player->mo->reactiontime = 2000;
}
...
{
	player->mo->flags2 &= ~MF2_REFLECTIVE;	
	P_SetMobjState(player->mo, S_POSS_CENTAUR);
	A_UnSetInvulnerable(player->mo);
...
{
	player->mo->special2 = (P_Random()&3)+5;
	S_StartSound(player->mo, SFX_BISHOP_ATTACK);	
}
...
void A_FireMacePL1B(player_t *player, pspdef_t *psp)
{
	...
	ball = P_SpawnMobj(pmo->x, pmo->y, pmo->z+28*FRACUNIT 
		...
		, MT_MACEFX2);
	ball->momz = 2*FRACUNIT+(((int)player->lookdir)<<(FRACBITS-5));
	angle = pmo->angle;
	ball->target = pmo;
	ball->angle = angle;
	ball->z += ((int)player->lookdir)<<(FRACBITS-4);
...
void A_FireMacePL2(player_t *player, pspdef_t *psp)
{
	...
	if(mo)
	{
		...
		mo->momz = 2*FRACUNIT+(((int)player->lookdir)<<(FRACBITS-5));
		if(linetarget)
		{
			mo->special1 = (int)linetarget;
		}
...
void A_FireCrossbowPL2(player_t *player, pspdef_t *psp)
{
	...
	P_SPMAngle(pmo, MT_CRBOWFX3, pmo->angle+(ANG45/5));
	S_StartSound(player->mo, sfx_bowsht);
}
...
void A_FireIceWand(player_t *player, pspdef_t *psp)
{
	angle_t angle;
	int damage;
	int slope;
	...
	if (!conedone)
	{
		mo = P_SpawnPlayerMissile(pmo, MT_BWANDFX1);
		if (mo)
		{
			mo->special1 = SHARDSPAWN_LEFT|SHARDSPAWN_DOWN|SHARDSPAWN_UP
				|SHARDSPAWN_RIGHT;
			mo->special2 = 0.2; // Set sperm count (levels of reproductivity)
			mo->target = pmo;
			mo->args[0] = 3;		// Mark Initial shard as super damage
			mo->damage = mo->damage*player->intelligence/10;
		}
