	...
	overRide)
{
	...
	pe = gi.Z_Malloc(sizeof(polyevent_t), PU_LEVSPEC, 0);
...
boolean EV_OpenPolyDoor(line_t *line, byte *args, podoortype_t type)
{
	...
	pd = gi.Z_Malloc(sizeof(polydoor_t), PU_LEVSPEC, 0);
		...
		for(i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++)
		{
			if(i >= 0 && i < bmapwidth && j >= 0 && j < bmapheight*bmapwidth)
			{
				...
				{ // Create a new link at the current block cell
					*link = gi.Z_Malloc(sizeof(polyblock_t), PU_LEVEL, 0);
					(*link)->next = NULL;
					(*link)->prev = NULL;
					(*link)->polyobj = po;
					continue;
				...
				{
					tempLink->next = gi.Z_Malloc(sizeof(polyblock_t), 
						PU_LEVEL, 0);
...
static void InitBlockMap(void)
{
	...
	PolyBlockMap = gi.Z_Malloc(bmapwidth*bmapheight*sizeof(polyblock_t *),
		PU_LEVEL, 0);
...
static void SpawnPolyobj(int index, int tag, boolean crush)
{
	...
	for(i = 0; i < numsegs; i++)
	{
			...
			segs[i].linedef->arg1 == tag)
		{
			...
			polyobjs[index].Segs = gi.Z_Malloc(PolySegCount*sizeof(seg_t *),
				PU_LEVEL, 0);
	...
	{ // didn't find a polyobj through PO_LINE_START
		...
		if(polyobjs[index].numSegs)
		{
			...
			polyobjs[index].crush = crush;
			polyobjs[index].tag = tag;
			polyobjs[index].Segs = gi.Z_Malloc(polyobjs[index].numSegs
...
static void TranslateToStartSpot(int tag, int originX, int originY)
{
	...
	po->originalPts = gi.Z_Malloc(po->numSegs*sizeof(vertex_t), PU_LEVEL, 0);
	po->prevPts = gi.Z_Malloc(po->numSegs*sizeof(vertex_t), PU_LEVEL, 0);
...
void PO_Init(int lump)
{
	...
	polyobjs = gi.Z_Malloc(po_NumPolyobjs*sizeof(polyobj_t), PU_LEVEL, 0);
