...
int R_LoadModel(char *filename)
{
	...
	for(i=0; i<mdl->header.numFrames; i++)
	{
		...
		mdl->frames[i].vertices = Z_Malloc(sizeof(md2_modelVertex_t) * 
...
modeldef_t *R_GetModelDef(char *name, int create_if_missing)
{
	...
	newlist = Z_Malloc(sizeof(modeldef_t) * (nummodels+1), PU_STATIC, NULL);
	if(nummodels)
	{
		// Copy the old list.
...
modelframe_t *R_GetModelDefFrame(modeldef_t *md, int frame)
{
	...
	newlist = Z_Malloc(sizeof(modelframe_t) * (frame+1), PU_STATIC, NULL);
