...
static void UnarchiveMobjs(void)
{
	...
	TargetPlayerAddrs = gi.Z_Malloc(MAX_TARGET_PLAYERS*sizeof(int *),
		PU_STATIC, NULL);
	TargetPlayerCount = 0;
	MobjCount = GET_LONG;
	...
	MobjList = gi.Z_Malloc(MobjCount*sizeof(mobj_t *), PU_STATIC, NULL);
	for(i = 0; i < MobjCount; i++)
	{
		MobjList[i] = gi.Z_Malloc(sizeof(mobj_t), PU_LEVEL, NULL);
	}
...
static void UnarchiveThinkers(void)
{
	...
	while((tClass = GET_BYTE) != TC_NULL)
	{
		for(info = ThinkerInfo; info->tClass != TC_NULL; info++)
		{
			...
			{
				thinker = gi.Z_Malloc(info->size, PU_LEVEL, NULL);
