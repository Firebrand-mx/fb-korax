...
void R_UpdateTextures (void)
{
	...
	maptex = maptex1 = W_CacheLumpName("TEXTURE1", PU_CACHE);
	numtextures1 = LONG(*maptex);
	if (W_CheckNumForName ("TEXTURE2") != -1)
	{
		maptex2 = W_CacheLumpName ("TEXTURE2", PU_CACHE);
		numtextures2 = LONG(*maptex2);
	}
	else
	{
	...
	//if(numtextures != numtextures1 + numtextures2)
	//{
		// We must update. Free the old data.
		/*for(i=0; i<numtextures; i++)
		{
			Z_Free(textures[i]);
...
{
	if(!r_texglow) return 0;
//	if (flat>numflats) flat=1;///Crap solution
//	if (flat<0) flat=1;
...
// translation table for global animation
	...
	// Flatflags uses TXF_*.
	flatflags = Z_Malloc((numflats+1)*4, PU_STATIC, 0);
...
void R_InitSpriteLumps (void)
{
	...
	spriteheight = Z_Malloc (numspritelumps*4, PU_REFRESHSPR, 0);
	spritewidth = Z_Malloc (numspritelumps*4, PU_REFRESHSPR, 0);
	spriteoffset = Z_Malloc (numspritelumps*4, PU_REFRESHSPR, 0);
	spritetopoffset = Z_Malloc (numspritelumps*4, PU_REFRESHSPR, 0);
...
//
	...
	colormaps = Z_Malloc (length, PU_REFRESHCM, 0);
	colormaps = (byte *)( ((int)colormaps + 255)&~0xff);
	W_ReadLump (lump,colormaps);*/
}


void R_UpdateColormaps (void)
{
/*	Z_FreeTags(PU_REFRESHCM, PU_REFRESHCM);
	R_InitColormaps();*/
}
...
{
	//printf( "UpdateTextures\n");
	R_UpdateTextures();
	//printf( "UpdateFlats\n");
	R_UpdateFlats();
	//printf( "UpdateSpriteLumps\n");
	R_UpdateSpriteLumps();
	//printf( "UpdateColormaps\n");
//	R_UpdateColormaps();
	//printf( "Done.\n");
}
...
//	
	...
	{
		flatpresent[SECTOR_PTR(i)->floorpic] = 1;
		flatpresent[SECTOR_PTR(i)->ceilingpic] = 1;
	}
