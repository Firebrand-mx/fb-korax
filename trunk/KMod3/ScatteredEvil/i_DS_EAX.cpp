...
int I2_Init()
{
	...
	if (FAILED(hr = dsound->Initialize(NULL)))
	{
		dsound->Release();
		dsound = NULL;
		ST_Message( "I2_Init: Couldn't initialize dsound (result: %i).\n", hr & 0xffff);
		return false;
	}
	// Set the cooperative level.
	if (FAILED(hr = dsound->SetCooperativeLevel(hWndMain, DSSCL_PRIORITY)))
	{
	...
	if (SUCCEEDED(dsound->CreateSoundBuffer(&desc, &bufTemp, NULL)))
	{
		// Query the listener interface.
		bufTemp->QueryInterface(IID_IDirectSound3DListener, (void **)&dsListener);
	}
	...
	if (SUCCEEDED( CreateDSBuffer(DSBCAPS_STATIC | DSBCAPS_CTRL3D, DSBSIZE_MIN, 22050, 8, 1, &bufTemp) ))
	{
		...
		if (SUCCEEDED(hr = bufTemp->QueryInterface(IID_IKsPropertySet, (void **)&eaxListener)))
		{
...
{
	if(src->source3D) src->source3D->Release();
	if(src->source) src->source->Release();
...
void I_StopSound(int handle)
{
	...
	{
		buf->source->Stop();
		buf->source->SetCurrentPosition(0);
	}
}
...
void I_UpdateListener(listener3d_t *desc)
{
	...
	if(desc->flags & DDLISTENERF_POS)
	{
		...
		dsListener->SetPosition(temp[VX], temp[VY], temp[VZ], DS3D_DEFERRED);
	}	
	if(desc->flags & DDLISTENERF_MOV)
	{
		for(i=0; i<3; i++) temp[i] = FIX2FLT(desc->mov[i]);
		dsListener->SetVelocity(temp[VX], temp[VY], temp[VZ], DS3D_DEFERRED);
	}
	if(desc->flags & DDLISTENERF_YAW || desc->flags & DDLISTENERF_PITCH)
	{
		...
		top[VZ] = -cos(y) * sin(p);
		top[VY] = cos(p);
		dsListener->SetOrientation(temp[VX], temp[VY], temp[VZ],	// Front vector.
...
void I2_SetPitch(sndsource_t *src, float pitch)
{
	...
	src->source->SetFrequency(newfreq);
}
...
void I2_UpdateSource(sndsource_t *buf, sound3d_t *desc)
{
	...
	{
		for(i=0; i<3; i++) temp[i] = FIX2FLT(desc->pos[i]);
		buf->source3D->SetPosition(temp[VX], temp[VY], temp[VZ], DS3D_DEFERRED);
	}
	if(desc->flags & DDSOUNDF_MOV)
	{
		for(i=0; i<3; i++) temp[i] = FIX2FLT(desc->mov[i]);
		buf->source3D->SetVelocity(temp[VX], temp[VY], temp[VZ], DS3D_DEFERRED);
	}
}
...
int I2_PlaySound(void *data, boolean play3d, sound3d_t *desc, int pan)
{
	...
	if(play3d)
	{
		...
		if(FAILED(hr = sndbuf->source->QueryInterface(IID_IDirectSound3DBuffer, 
			(void **)&sndbuf->source3D)))
		{
			ST_Message( "I2_PlaySound: couldn't get 3D buffer interface (result = %d).\n", hr & 0xffff);
			sndbuf->source->Release();
			memset(sndbuf, 0, sizeof(*sndbuf));
			return 0;
		}
	}
	...
	// Lock the buffer.
	if(FAILED(hr = sndbuf->source->Lock(0, samplelen, 
	...
	// Unlock the buffer.
	if(FAILED(hr = sndbuf->source->Unlock(writePtr1, writeBytes1, writePtr2, writeBytes2)))
	{
	...
	if(play3d)
	{
		...
		if(desc->flags & DDSOUNDF_VERY_LOUD)
		{
			...
			sndbuf->source3D->SetMinDistance(10000, DS3D_DEFERRED);
			sndbuf->source3D->SetMaxDistance(20000, DS3D_DEFERRED);	
		}
		else
		{
			sndbuf->source3D->SetMinDistance(100, DS3D_DEFERRED);
			sndbuf->source3D->SetMaxDistance(MAX_SND_DIST, DS3D_DEFERRED);
		}
		if(desc->flags & DDSOUNDF_LOCAL)
			sndbuf->source3D->SetMode(DS3DMODE_DISABLE, DS3D_DEFERRED);
	}
	else
	{
	...
	if(FAILED(hr = sndbuf->source->Play(0, 0, 0)))
	{
...
void I_Update2DSound(int handle, int volume, int pan, int pitch)
{
	...
	if(!I2_IsSourcePlaying(buf)) buf->source->Play(0, 0, 0);
