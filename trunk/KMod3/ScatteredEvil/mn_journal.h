...
class KJournalEnemiesPage:public KJournalPageBaseWindow
{
	...
	void CreatePages(void)
	{
		...
		AddPage("WASTELAND CHAOS SERPENT", "DEM2G1", "A BROWN MENACE OF MOST DESERTS\n"
			...
			"HIT POINTS:\n500\n\nATTACK DAMAGE:\n10-80");
		AddPage("STALKERS", "SSPTJ2J8", "BLAH\n"
			"BLAH\n"
			"BLAH.",
			"HIT POINTS:\n250\n\nATTACK DAMAGE:\n5-40");
		AddPage("DEATH WYVERN", "DRAGB2B8", "AN UNDEAD DRAGON, ITS THIRST\n"
			...
			"HIT POINTS:\n640\n\nATTACK DAMAGE:\nUP TO 128");
		AddPage("ZEDAC", "PLAYE8", "BLAH\n"
			"BLAH\n"
			"BLAH.",
			"HIT POINTS:\n250\n\nATTACK DAMAGE:\n5-40");
		AddPage("MENELKIR", "MAGEF8", "BLAH\n"
			"BLAH\n"
			"BLAH.",
			"HIT POINTS:\n250\n\nATTACK DAMAGE:\n5-40");
		AddPage("TRADUCTUS", "CLERB8", "BLAH\n"
			"BLAH\n"
			"BLAH.",
			"HIT POINTS:\n250\n\nATTACK DAMAGE:\n5-40");
	}
...
class KJournalItemsPage:public KJournalPageBaseWindow
{
	...
	void CreatePages(void)
	{
		...
		AddPage("GREEN MANA", "MAN2A0", "THIS MAGICAL ITEM INCREASES\n"
...
class KJournalQuestsPage:public KJournalPageBaseWindow
{
	DECLARE_CLASS(KJournalQuestsPage, KJournalPageBaseWindow, 0);
	NO_DEFAULT_CONSTRUCTOR(KJournalQuestsPage);

	struct FJournalQuestsPage
	{
		char *Title;
		char *Quest;
		char *Description;
	...
	KTextWindow *winTitle;
	KTextWindow *winQuest;
	KJournalDescriptionWindow *winDesc;

	int CurrentPage;
	int NumPages;
	FJournalQuestsPage Pages[64];
	...
	{
		winQuest = NewWindow(KTextWindow, this);
		winQuest->SetPos(0, 35);
		winQuest->SetSize(240, 12);
		winQuest->SetFont(KCanvas::SmallFont);
		winQuest->SetTextAlignments(HALIGN_Center, VALIGN_Center);
	}
	...
	void CreateDescWindow(void)
	{
		winDesc = NewWindow(KJournalDescriptionWindow, this);
		winDesc->SetPos(0, 60);
		winDesc->SetSize(240, 90);
	}

	void AddPage(char *Quest, char *Description)
	{
		if (NumPages >= 64)
		{
			I_Error("Too many item pages");
		}
		Pages[NumPages].Quest = Quest;
		Pages[NumPages].Description = Description;
		NumPages++;
	}
	...
	void CreatePages(void)
	{
		AddPage("OLD MS NORRIS' SHOPING", "you have to get bread and wine\n"
			"to old ms NORRIS, who is too\n"
			"old and sick TO DO SO HER SELF.");
		AddPage("OLD MS NORRIS' CAT", "MS NORRIS' CAT IS STUCK IN\n"
			"A TREE, AND THE OLD DEAR IS TOO\n"
			"OLD TO CLIMB UP AND GET HIM.");
	}

	void SetActivePage(void)
	{
		winQuest->SetText(Pages[CurrentPage].Quest);
		winDesc->SetText(Pages[CurrentPage].Description);
	}
...
IMPLEMENT_CLASS(KJournalQuestsPage);
...
class KJournalAttrPage:public KJournalPageBaseWindow
{
	DECLARE_CLASS(KJournalAttrPage, KJournalPageBaseWindow, 0);
	NO_DEFAULT_CONSTRUCTOR(KJournalAttrPage);
	...
	KTextWindow *winTitle;
	KTextWindow *winHead;
	KTextWindow *winRemain;
/*	KMenuUIChoice *winScroller;*/
	KJournalDescriptionWindow *winPnts;
	KTextWindow *winButton;
...
/*	KJournalScroller(void)
	...
	void CreateTitle(void)
	{
		...
		winTitle->SetPos(0, 18);
		winTitle->SetSize(240, 12);
		winTitle->SetText("YOUR ATTRIBUTES");
		winTitle->SetFont(KCanvas::YellowFont);
		winTitle->SetTextAlignments(HALIGN_Center, VALIGN_Center);
	}
	...
	{
		winHead = NewWindow(KTextWindow, this);
		winHead->SetPos(0, 42);
		winHead->SetSize(240, 12);
		winHead->SetText("POINTS:           OLD     NEW");
		winHead->SetFont(KCanvas::SmallFont);
		winHead->SetTextAlignments(HALIGN_Center, VALIGN_Center);
	}

	void CreateRemaining(void)
	{
		char tmp[16];

		sprintf(tmp, "REMAINING: " "%02d" "  ", Value);

		winRemain = NewWindow(KTextWindow, this);
		winRemain->SetPos(0, 150);
		winRemain->SetSize(240, 12);
		winRemain->SetText(tmp);
		winRemain->SetFont(KCanvas::SmallFont);
		winRemain->SetTextAlignments(HALIGN_Right, VALIGN_Center);
	}

	void CreatePoints(KGC *gc)
	{
/*		char tmp[16];
...
IMPLEMENT_CLASS(KJournalAttrPage);
...
class KJournalScreen:public KMenuScreen
{
	...
	KJournalPageBaseWindow *winJournalPages[6];
	...
	void InitWindow(void)
	{
		...
		winJournalPages[5] = NewWindow(KJournalAttrPage, this);
		winJournalPages[5]->Hide();
	}

	void DrawWindow(KGC *gc)
	{
		static char* journal[6]=
		{
			{"STATS"},
			{"BESTIARY"},
			{"THINGS"},
	...
	bool KeyPressed(int key)
	{
		...
		if (key == DDKEY_PGDN)
		{
			...
			currentJournal = currentJournal == 5 ? 0 : currentJournal + 1;
		...
		{
			winJournalPages[currentJournal]->Hide();
			currentJournal = currentJournal == 0 ? 5 : currentJournal - 1;
			winJournalPages[currentJournal]->Show();
			SetFocusWindow(winJournalPages[currentJournal]);
			return true;
		}
