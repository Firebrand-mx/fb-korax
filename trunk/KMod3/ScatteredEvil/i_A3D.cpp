...
//
//	Directly from A3D utils.
//
static bool A3dRegister(void)
{
	...
	if (hA3dDLL == NULL)
	{
		ST_Message("IA3dUtil: Failed to load the A3D.DLL for registration.\n");
		return false;
	}
	...
	{
		ST_Message("IA3dUtil: Failed to get the DllRegisterServer() procedure address from the A3D.DLL.\n");
		return false;
	}
	...
	if (FAILED(hResult))
	{
		ST_Message("IA3dUtil: Failed to Register the A3D.DLL library.\n");
		return false;
	}
	...
	if (hA3dDLL == NULL)
	{
		ST_Message("IA3dUtil: Failed to load the A3DAPI.DLL for registration.\n");
		return false;
	}
	...
	if (lpfnDllRegisterServer == NULL)
	{
		ST_Message("IA3dUtil: Failed to get the DllRegisterServer() procedure address from the A3DAPI.DLL.\n");
		return false;
	}
	...
	if (FAILED(hResult))
	{
		ST_Message("IA3dUtil: Failed to Register the A3DAPI.DLL library.\n");
		return false;
	}
...
int I3_Init()
{
	...
	if (!A3dRegister()) return false;
	// COM create the A3d root interface.
	if (FAILED(hr = CoCreateInstance(CLSID_A3dApi, NULL, CLSCTX_INPROC_SERVER, 
		IID_IA3d5, (void**)&a3d)) || !a3d)
	{
		ST_Message( "I3_Init: Couldn't create a3d (result = %d).\n", hr&0xffff);
		return false;
	}
	// Initialize A3D.
	if (FAILED(hr = a3d->Init(NULL, A3D_REVERB, A3DRENDERPREFS_DEFAULT)))
	{
		a3d->Release();
		ST_Message( "I3_Init: Couldn't init a3d (result = %d).\n", hr&0xffff);
		return false;
	}
