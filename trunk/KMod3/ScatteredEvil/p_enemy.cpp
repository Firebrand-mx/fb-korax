...
void A_PossBishWeapCheck(mobj_t *actor)
{
	if (actor->player->readyweapon == WP_BISHOP_MELEE)
		P_SetMobjState(actor, S_POSS_BISHOP);
}
...
void A_DontMove(mobj_t *actor)
{
	actor->reactiontime = 255;
}
...
boolean P_LookForMonsters(mobj_t *actor)
{
	...
	for(think = thinkercap.next; think != &thinkercap; think = think->next)
	{
		...
		if(actor->flags3&MF3_FRIENDLY && (mo->flags3&MF3_FRIENDLY || mo->player))
		{ //Don't attack one of your own
			continue;
		}
...
boolean P_LookForPlayers(mobj_t *actor, boolean allaround)
{
	...
	if(actor->flags3&MF3_FRIENDLY && P_LookForMonsters(actor))
	{ //Friendly monsters search for bad monsters before attempting to follow player
		return(true);
	}
	
	if (actor->flags3&MF3_FRIENDLY) //Friendly monsters look for players differently
	{
		dist = 0;		
	...
	for( ; ; actor->lastlook = (actor->lastlook+1)&3 ) //Normal monster looks for player code
	{
		...
		{ //Try to target player possession dummy
			mo = ActivePlayerDummy(player);
			if (P_CheckSight (actor, mo))
			{
				actor->target = mo;
				return(true);
...
void A_Chase(mobj_t *actor)
{
	...
	if (actor->player) //Possessed Monsters are players and do stop by into this void
	{
		return;
	}
...
//
	if (actor->info->meleestate && P_CheckMeleeRange (actor)
		&& !(actor->flags3&MF3_FRIENDLY && actor->target->player)) //Don't attack friendly player
	{
...
//
	if ((netgame && !actor->threshold && !P_CheckSight (actor, actor->target))
		|| (actor->flags3&MF3_FRIENDLY && actor->target->player && !actor->threshold))
	{
		if (P_LookForPlayers(actor,true))
...
void A_SetReflective(mobj_t *actor)
{
	...
	if ((actor->type == MT_CENTAUR) ||
		...
		(actor->type == MT_SCENTAUR) ||
		(actor->type == MT_SCENTAURLEADER))
	{
...
void A_Scream(mobj_t *actor)
{
	...
	if(actor->player)
	{
		if(actor->player->morphTics || actor->player->pclass >= PCLASS_ETTIN) //Remi
		{
...
//	P_DropMoney
//
//	-JL- Seperate function used also by ice death
//
//----------------------------------------------------------------------------

void P_DropMoney(mobj_t *actor)
{
	int type;

	// Check for monsters dropping things
	type = actor->type;
	// -JL- Pigs drops what original monster would drop
	if (type == MT_PIG)
	{
		type = actor->special2;
	}
	switch(type)
	{
		...
		/*case MT_MUMMYLEADERGHOST:
			...
			break;*/
		case MT_ETTIN:
			P_DropItem(actor, MT_SILVER, 1, 64);
			break;
		case MT_SCENTAUR:
		case MT_CENTAUR:
			P_DropItem(actor, MT_SILVER, 1, 128);
			break;
		case MT_SCENTAURLEADER:
		case MT_CENTAURLEADER:
        case MT_BISHOP:
			P_DropItem(actor, MT_GOLD, 1, 64);
			break;
        case MT_FIGHTER_BOSS:
        case MT_CLERIC_BOSS:
        case MT_MAGE_BOSS:
			P_DropItem(actor, MT_GOLD, 1, 128);
			break;
		case MT_SORCBOSS:
			P_DropItem(actor, MT_GOLD, 1, 300);
			break;
        default:
			break;
	}
...
void P_InitCreatureCorpseQueue(boolean corpseScan)
{
	...
	for(think = thinkercap.next; think != &thinkercap; think = think->next)
	{
		...
		switch(mo->type)
		{
			...
			case MT_DEMON3:
			case MT_DEMON3CHUNK1:
			case MT_DEMON3CHUNK2:
			case MT_DEMON3CHUNK3:
			case MT_DEMON3CHUNK4:
			case MT_DEMON3CHUNK5:
			case MT_SCENTAUR:
			case MT_SCENTAURLEADER:
			case MT_SCENTAUR_SHIELD:
			case MT_SCENTAUR_SWORD:
...
void A_SerpentHumpDecide(mobj_t *actor)
{
	if(actor->type == MT_SERPENTLEADER)
	{
		...
		{ // Missile attack
			if (actor->health < actor->info->spawnhealth / 2)
					{
						P_SetMobjState(actor, S_DAMAGED_SERPENT_SURFACE1);
					}
					else
					{
	...
	{ // The hump shouldn't occur when within melee range
		...
		{	
			if (actor->health < actor->info->spawnhealth / 2)
					{
						P_SetMobjState(actor, S_DAMAGED_SERPENT_HUMP1);
					}
					else
					{
...
//
	if (actor->info->meleestate && P_CheckMeleeRange (actor))
	{
		...
		if (actor->health < actor->info->spawnhealth / 2)
					{
						P_SetMobjState(actor, S_DAMAGED_SERPENT_ATK1);
					}
					else
					{
...
void A_SerpentCheckForAttack(mobj_t *actor)
{
	...
	if(P_CheckMeleeRange2(actor))
	{
		if (actor->health < actor->info->spawnhealth / 2)
					{
						P_SetMobjState(actor, S_DAMAGED_SERPENT_WALK1);
					}
					else
					{
...
void A_SerpentChooseAttack(mobj_t *actor)
{
	...
	if(actor->type == MT_SERPENTLEADER)
	{
		if (actor->health < actor->info->spawnhealth / 2)
					{
						P_SetMobjState(actor, S_DAMAGED_SERPENT_MISSILE1);
					}
					else
					{
...
{
	if (!actor->player) //Possessed Bishop enters here
	{
...
//	actor->special1 = (weaveindex+2)&63;
	...
	if ((actor->z+75*FRACUNIT) > actor->ceilingz)
	{
		actor->z -= 2*FRACUNIT;
	}
		...
		} else*/
		{
			if (P_Random()<30)
			{
				...
				if (P_Random()>200)
				    ...
				    if (P_Random()>200)
						if (ang>ANGLE_180) ang -= ANGLE_180;
						else ang += ANGLE_180;
/*				else
					ang -= ANGLE_90;*/
				ang>>=ANGLETOFINESHIFT;
...
/*
	DragonSeek(actor, 4*ANGLE_1, 20*ANGLE_1);
	if(actor->target)
	{
...
void A_DragonPain(mobj_t *actor)
{
	A_Pain(actor);
	if(!actor->special1)
		if (actor->health < actor->info->spawnhealth / 2)// no destination spot yet
		{
			P_SetMobjState(actor, S_DRAGON_DAMAGED_INIT);
		}
		else
		{
...
//Added by Korax Project
void A_Demon2Attack1(mobj_t *actor)
{
	if(P_CheckMeleeRange(actor))
	{
		P_DamageMobj(actor->target, actor, actor, HITDICE(4));
...
void A_WraithRaise(mobj_t *actor)
{
	if (A_RaiseMobj(actor))
	{
		...
		if (actor->health < actor->info->spawnhealth / 2)
		{
			P_SetMobjStateNF(actor, S_DAMAGED_WRAITH_CHASE1);
		}
		else
		{
			P_SetMobjStateNF(actor, S_WRAITH_CHASE1);
		}
	}
...
{
	int weaveindex = actor->special1;
	if (!actor->player) //Screws up movement for possessed player
	{
		actor->z += FloatBobOffsets[weaveindex];
		actor->special1 = (weaveindex+2)&63;
	}
...
//	}
	A_Chase(actor);
	if (!actor->player) A_WraithFX4(actor); //Remi
}
...
void A_FiredChase(mobj_t *actor)
{
	...
	// Do missile attack
	if (!(actor->flags&MF_JUSTATTACKED)
		&& !(actor->flags3&MF3_FRIENDLY && actor->target->player)) //Don't attack friendly monster
	{
		if (P_CheckMissileRange(actor) && (P_Random()<20))
		{
			P_SetMobjState (actor, P_GetMissileState(actor));
	...
	//Friendly monsters tries to target new monster while following player
	if (actor->flags3&MF3_FRIENDLY && actor->target->player && !actor->threshold)
	{
		if (P_LookForPlayers(actor,true))
			return;         // got a new target
	}
	...
	switch(mode)
	{
		...
		case SORC_FIRESPELL:			// Casting spell
			if (parent->special2 == actor->type)
			{
				...
				if (parent->health > 0)
					if (parent->health < parent->info->spawnhealth / 2)
					{
						P_SetMobjStateNF(parent, S_SORC_DAMAGED_ATTACK1);
					}
					else
					{
			...
			if (parent->special2 == actor->type)
			{
				if (actor->special2-- <= 0)
				{
					...
					if (parent->health > 0)
						if (parent->health < parent->info->spawnhealth / 2)
						{
							P_SetMobjStateNF(parent, S_SORC_DAMAGED_ATTACK4);	
						}
						else
						{
...
{
	mobj_t *mo;
	mo=P_SpawnMobj(actor->x, actor->y, actor->z, (P_Random()>(gameskill<2?253:(gameskill<3?245:230))?MT_SORCBOSS:MT_BISHOP));
	if(mo)
	{
...
void A_FreezeDeathChunks(mobj_t *actor)
{
	...
	{ // attach the player's view to a chunk of ice
		mo = P_SpawnMobj(actor->x, actor->y, actor->z+actor->player->misc_viewheight, MT_ICECHUNK);
...
void KoraxFire1(mobj_t *actor, mobjtype_t type);
void KoraxFire2(mobj_t *actor, mobjtype_t type);
void KoraxFire3(mobj_t *actor, mobjtype_t type);
void KoraxFire4(mobj_t *actor, mobjtype_t type);
void KoraxFire5(mobj_t *actor, mobjtype_t type);
void KoraxFire6(mobj_t *actor, mobjtype_t type);
...
void A_Demon3Death(mobj_t *actor)
{
	mobj_t *mo;
	angle_t angle;
...
//================================================================================================
//
// A_Demon3Attack1 (melee)
//
//================================================================================================

void A_Demon3Attack1(mobj_t *actor)
{
	if(P_CheckMeleeRange(actor))
	{
		P_DamageMobj(actor->target, actor, actor, HITDICE(2));
...
//================================================================================================
//
// A_Demon3Attack2 (missile)
//
//================================================================================================

void A_Demon3Attack2(mobj_t *actor)
{
	mobj_t *mo;
	mobjtype_t fireBall;

	if(actor->type == MT_DEMON3)
	{
		fireBall = MT_DEMON3FX1;
	}
	else
	{
		fireBall = MT_DEMON3FX1;
...
void A_SCentaurAttack(mobj_t *actor)
{
	if(!actor->target)
	{
		return;
...
void A_SCentaurAttack2(mobj_t *actor)
{
	if(!actor->target)
	{
		return;
	}
	P_SpawnMissile(actor, actor->target, MT_SCENTAUR_FX);
	S_StartSound(actor, SFX_CENTAURLEADER_ATTACK);
}
...
void A_SCentaurDropStuff(mobj_t *actor)
{
	mobj_t *mo;
	angle_t angle;
...
void A_SCentaurDie(mobj_t *actor)
{
	void A_StoneDeathChunks(mobj_t *actor);
	void A_SCentaurDropStuff(mobj_t *actor);
	...
	actor->height <<= 2;
	actor->angle = 2;
	A_StoneDeathChunks(actor);
	A_SCentaurDropStuff(actor);
}
...
void A_StoneDeathChunks(mobj_t *actor)
{
	...
	for(i = 12+(P_Random()&15); i >= 0; i--)
	{
		mo = P_SpawnMobj(actor->x+(((P_Random()-128)*actor->radius)>>10), 
			actor->y+(((P_Random()-128)*actor->radius)>>10), 
			actor->z+(P_Random()*actor->height/255), MT_STONECHUNK);
		P_SetMobjState(mo, mo->info->spawnstate+(P_Random()%3));
		if(mo)
		{
			mo->momz = FixedDiv(mo->z-actor->z, actor->height)<<2;
...
//============================================================================
//
// A_StoneCheckHeadDone
//
//============================================================================

void A_StoneCheckHeadDone(mobj_t *actor)
{
	if(actor->special2 == 666)
	{
		P_SetMobjState(actor, S_STONECHUNK_HEAD2);
	}
}
...
void A_StoneGuyAttack(mobj_t *actor)
{
	fixed_t an;
...
//============================================================================
//
// A_StoneGuyMissilePuff
//
//============================================================================

void A_StoneGuyMissilePuff(mobj_t *actor)
{
	mobj_t *mo;
	mo = P_SpawnMobj(actor->x, actor->y, actor->z+2*FRACUNIT, MT_STONEFX_PUFF);
}
...
void A_StoneGuyDie(mobj_t *actor)
{
	void A_StoneDeathChunks(mobj_t *actor);
...
//============================================================================
//
// A_StoneGuyMissileExplode
//
//============================================================================

void A_StoneGuyMissileExplode(mobj_t *actor)
{
	mobj_t *mo;
	int i;
	...
	for(i = 0; i < 8; i++)
	{
		mo = P_SpawnMissileAngle(actor, MT_STONEGUY_FX2, i*ANG45,	(int) (-0.3*FRACUNIT));
		if(mo)
		{
			mo->target = actor->target;
		}
...
void A_BeastAttack(mobj_t *actor)
{
	...
	P_SpawnMissile(actor, actor->target, MT_SHADOWBALL);
}