...
#include "i_timer.h"
#include "console.h"
#include "jtNet.h" // Exported functions from jtNet2.dll.
...
void I_NetPlayerArrives(int newnode)
{
	...
	for(i=0; i<MAXPLAYERS; i++)
		if(dcNodes[i].jtNetNode == newnode)
		{
			...
			H2_NetPlayerEvent(dcNodes[i].consoleplayer, DDPE_ARRIVAL, NULL);
			break;
		}
}
...
void I_NetPlayerLeaves(jtnetplayer_t *plrdata)
{
	...
	H2_NetPlayerEvent(pNumber, DDPE_EXIT, NULL);
}
...
int CCmdNet(int argc, char **argv)
{
	...
	if(argc == 2)	// One argument?
	{
		...
		else if(!stricmp(argv[1], "servers"))
		{
			...
			{
				jtnetserver_t *buffer = (jtnetserver_t *)malloc(sizeof(jtnetserver_t) * num);
				// -JL- Paranoia
				if (!buffer)
