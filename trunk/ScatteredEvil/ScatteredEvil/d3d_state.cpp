...
void initState()
{
	...
	ST_Message( "Supported texture sizes: min: %ix%i, max: %ix%i\n",
	...
	if(devdesc.dwMinTextureWidth > 1 || devdesc.dwMinTextureHeight > 1)
		ST_Message( "Warning: No 1x1 textures. You may encounter problems.\n");
	if(devdesc.dwMaxTextureWidth != devdesc.dwMaxTextureHeight)
	{
		ST_Message( "Warning: %i is considered the maximum size for width AND height.\n",
			maxTexSize);
	}
	...
	if(gammaSupport)
	{
		ST_Message( "Driver supports dynamic gamma ramps for primary surface.\n");
	}
	unguard;
}
