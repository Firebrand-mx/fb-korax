...
void ReadConfig(void)
{
	...
	GetPrivateProfileString("drD3D", "Driver", "", wantedDriver, 40, 
		"./ScatteredEvil.ini");
	wantedBits = GetPrivateProfileInt("drD3D", "Color", 16, "./ScatteredEvil.ini");
	wantedTexDepth = GetPrivateProfileInt("drD3D", "Texture", 0, "./ScatteredEvil.ini");
	wantedZDepth = GetPrivateProfileInt("drD3D", "ZBuffer", 16, "./ScatteredEvil.ini");
	showconf = GetPrivateProfileInt("drD3D", "CfgNextTime", 1, "./ScatteredEvil.ini");

	if(showconf 
		|| (GetAsyncKeyState(VK_SHIFT)
			| GetAsyncKeyState(VK_CONTROL)
			| GetAsyncKeyState(VK_MENU)) & 0x8000
		|| M_CheckParm("-d3dcfg") /*|| Shift/Ctrl/Alt pressed */)
	{
		// Automagically disable config dialog for the next time.
		WritePrivateProfileString("drD3D", "CfgNextTime", "0", "./ScatteredEvil.ini");
		...
		if(ConfigDialog())
		{
			...
			WritePrivateProfileString("drD3D", "Color", buf, "./ScatteredEvil.ini");
			sprintf(buf, "%i", wantedTexDepth);
			WritePrivateProfileString("drD3D", "Texture", buf, "./ScatteredEvil.ini");
			sprintf(buf, "%i", wantedZDepth);
			WritePrivateProfileString("drD3D", "ZBuffer", buf, "./ScatteredEvil.ini");
		}		
	}
	unguard;
}
							 ...
							 LPSTR lpDriverName, void *lpContext, HMONITOR hm)
{
	...
	ST_Message( "3dAccel:%i 3d:%i wndRend:%i zbuf:%i\n", 
...
void printPixFormat(DDPIXELFORMAT *pf)
{
	...
	ST_Message( "flags: (");
	if(pf->dwFlags & DDPF_RGB) ST_Message( "Rgb");
	if(pf->dwFlags & DDPF_ALPHAPIXELS) ST_Message( "Alpha");
	if(pf->dwFlags & DDPF_PALETTEINDEXED8) ST_Message( "Pal8");
	ST_Message( ") ");
	ST_Message( "bits:%i R:%x G:%x B:%x ",
		pf->dwRGBBitCount, pf->dwRBitMask, pf->dwGBitMask, pf->dwBBitMask);
	if(pf->dwFlags & DDPF_ALPHAPIXELS)
		ST_Message("A:%x", pf->dwRGBAlphaBitMask);
	ST_Message( "\n");
	unguard;
}
...
int dxInit(int firstTime)
{
	...
	if((i = M_CheckParm("-texbits")) && i < Argc()-1)
		wantedTexDepth = atoi(Argv(i+1));
	if((i = M_CheckParm("-zbits")) && i < Argc()-1)
		wantedZDepth = atoi(Argv(i+1));
	if((i = M_CheckParm("-driver")) && i < Argc()-1)
		strcpy(wantedDriver, Argv(i+1));
	if(M_CheckParm("-badtexalpha")) wantGoodAlpha = DGL_FALSE;

	if(firstTime)
	{
		ST_Message( "drD3D.dxInit: Enumerating DirectDraw devices.\n");
		...
		if(!found)
		{
			ST_Message( "drD3D.dxInit: No suitable DirectDraw devices found.\n");
			return DGL_ERROR;
		}
	}
	else
	...
	{
		ST_Message("drD3D.dxInit: SetCooperativeLevel failed. Result:%i.\n", hr&0xffff);
		return DGL_ERROR;
	}
	...
	if(windowed)
	{
		...
		{
			ST_Message( "drD3D.dxInit: Desktop color depth must be used in windowed mode.\n");
			return DGL_ERROR;
		}
	}
	else
	{
		...
		if(FAILED(hr = IDirectDraw4_SetDisplayMode(ddraw, screenWidth, screenHeight,
			wantedBits, 0, 0)))
		{
			ST_Message( "drD3D.dxInit: Failed to change display mode to %i x %i x %i. Result:%i.\n",
	...
	else // Fullscreen?
	{
		...
		if(FAILED(hr = IDirectDraw4_CreateSurface(ddraw, &sd, &sPrimary, NULL)))
		{
			ST_Message( "drD3D.dxInit: Failed to create the primary surface. Error: %i.\n", hr & 0xffff);
			return DGL_ERROR;
		}
		...
		if(FAILED(hr = IDirectDrawSurface4_GetAttachedSurface(sPrimary,
			&caps, &sBack)))
		{
			ST_Message( "drD3D.dxInit: Failed to get/create the backbuffer. Error: %i.\n", hr & 0xffff);
			return DGL_ERROR;
		}
	}
	...
	if(firstTime)
	{
		ST_Message( "drD3D.dxInit: Enumerating z-buffer pixel formats.\n");
		...
		if(!found)
		{
			ST_Message( "drD3D.dxInit: Couldn't find a suitable z-buffer pixel format.\n");
			return DGL_ERROR;
		}
	}
	...
	{
		ST_Message( "drD3D.dxInit: Z-buffer creation failed. Result: %i.\n",
			hr & 0xffff);
		return DGL_ERROR;
	}
	...
	if(FAILED(hr = IDirect3D3_CreateDevice(d3d, IID_IDirect3DHALDevice, sBack, 
		&d3dDevice, NULL)))
	{
		ST_Message( "drD3D.dxInit: D3D device creation failed. Result: %i.\n",
			hr & 0xffff);
		return DGL_ERROR;
	}
	...
	if(firstTime)
	{
		ST_Message( "drD3D.dxInit: Enumerating texture formats.\n");
		...
		{
			ST_Message( "drD3D.dxInit: Couldn't find suitable texture formats!\n");
			return DGL_ERROR;
		}
		if(found & TXFOUND_PAL8) palTexAvailable = DGL_TRUE;

		// Print a report.
		ST_Message( "Chosen texture formats:\n");
		ST_Message( "RGB:  ");
	...
	if(gammaSupport)
	{
		...
		{
			ST_Message( "Query for gamma control failed!\n");
...
/*	LPDIRECTDRAWSURFACE4	oldPrimary = sPrimary;
	...
	ST_Message("Setting render target to %p.\n", sBack);
