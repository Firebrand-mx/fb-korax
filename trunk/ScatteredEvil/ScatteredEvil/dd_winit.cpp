...
static void InitDrivers(cmdargs_t *cargs)
{
	...
	showconf = GetPrivateProfileInt("Drivers", "CfgNextTime", 1, "./ScatteredEvil.ini");
	gfxDriver = GetPrivateProfileInt("Drivers", "GfxDriver", 0, "./ScatteredEvil.ini");
	use_jtSound = GetPrivateProfileInt("Drivers", "SndDriver", 1, "./ScatteredEvil.ini");
		...
		GetAsyncKeyState(VK_CONTROL) | GetAsyncKeyState(VK_MENU)) & 0x8000)
	{
		...
		WritePrivateProfileString("Drivers", "CfgNextTime", "0", "./ScatteredEvil.ini");
		...
		if (DialogBox(hInstApp, MAKEINTRESOURCE(IDD_DRVCONFIG), NULL, ConfigDialogProc))
		{
			...
			WritePrivateProfileString("Drivers", "GfxDriver", buf, "./ScatteredEvil.ini");
			sprintf(buf, "%i", use_jtSound);
			WritePrivateProfileString("Drivers", "SndDriver", buf, "./ScatteredEvil.ini");
		}		
	}
		...
		LPSTR lpCmdLine, int nCmdShow)
{
	...
	newout = freopen("ScatteredEvil.log", "w", stdout);
	if(!newout)
	{
		MessageBox(NULL, 
			"Redirection of stdout to ScatteredEvil.log failed. You won't see anything that's printf()ed.",
