...
void A_PossProjSpawn(mobj_t *actor) //Sister action, used by Possession Projectile
{
	mobj_t *mo;

	if (actor->type == MT_POSS_PROJECTILE2) //The tail shall not spawn itself, nor shall it live
	{
		P_SetMobjState(actor, actor->info->deathstate);
		return;
	}
	
	mo = P_SpawnMobj(actor->x, actor->y, actor->z, MT_POSS_PROJECTILE2);
	if (mo)
	{
		mo->special1 = TELEPORT_LIFE;			// Lifetime countdown
		mo->angle = actor->angle;
...
void A_SummonMonster(mobj_t *origin)
{
	mobj_t *mo;
	mobj_t *pmo;
	mobjtype_t motype;
	...
	if (i>=200 && i<=256)
		...
		motype = MT_CENTAURLEADER;
	else if (i>=70 && i<=79)
		motype = MT_WRAITH;
	else if (i>=60 && i<=69)
		motype = MT_BISHOP;
	else motype = MT_ETTIN;
	
	mo = P_SpawnMobj(origin->x, origin->y, origin->z, motype);
	
	if (mo)
		if(P_TestMobjLocation(mo) == false) //Shitto! Didn't fit.
		{
			P_SetMobjState(mo, S_NULL);
			P_SetMessage(player, "SUMMON SPELL FAILED", false);
			return;
		}
	P_SpawnMobj(mo->x, mo->y, mo->z, MT_MNTRSMOKE);
	if (P_Random()>15)
	{
		P_SetMessage(player, "SUMMON SPELL ACTIVATED", false);
		mo->flags3 |= MF3_FRIENDLY;
	}
	else P_SetMessage(player, "SUMMON SPELL BACKFIRED", false);
