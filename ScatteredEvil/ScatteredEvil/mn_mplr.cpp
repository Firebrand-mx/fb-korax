...
void SCEnterHostMenu(int option);
void SCEnterJoinMenu(int option);
void SCEnterGameSetup(int option);
void SCSetProtocol(int option);
void SCGameSetupFunc(int option);
void SCGameSetupMission(int option);
void SCGameSetupSkill(int option);
void SCGameSetupDamageMod(int option);
void SCGameSetupHealthMod(int option);
void SCOpenServer(int option);
void SCCloseServer(int option);
void SCChooseHost(int option);
void SCStartStopDisconnect(int option);
void SCEnterPlayerSetupMenu(int option);
void SCPlayerClass(int option);
void SCPlayerColor(int option);
void SCAcceptPlayer(int option);
void SCComPort(int option);
void SCBaudRate(int option);
void SCStopBits(int option);
void SCParity(int option);
void SCFlowControl(int option);
void SCModemSelector(int option);

void ResetJoinMenuItems();

// Edit fields.
void DrawEditField(Menu_t *menu, int index, EditField_t *ef);
void SCEditField(int efptr);
...
MenuItem_t GameSetupItems[] = 
{
	...
	{ ITT_EFUNC, "RANDOM CLASSES:", SCGameSetupFunc, (int) &netRandomclass, MENU_NONE },
...
void DrawGameSetupMenu(void)
{
	char	*mapName = P_GetMapName(P_TranslateMap(netMap));
	...
	MN_DrTextAYellow_CS(skillText[netSkill], x, y+=h);
	MN_DrTextAYellow_CS(boolText[netDeathmatch], x, y+=h);
	MN_DrTextAYellow_CS(boolText[!netNomonsters], x, y+=h);
	MN_DrTextAYellow_CS(boolText[netRandomclass], x, y+=h);
	DrANumber(netMobDamageModifier, x, y+=h);
	DrANumber(netMobHealthModifier, x, y+=h);
}
...
void DrawPlayerList(int y)
{
	...
	for(i=0, y+=9; i<MAXPLAYERS; i++)
	{
		...
		MN_DrTextA_CS(classText[plrInfo[i].pclass], col[1], y);
...
void DrawGameSetupInfo(int y)
{
	...
	sprintf(buf, "MAP %i, %s%s", netMap, skillText[netSkill],
		netRandomclass? ", RANDOM CLASSES" : "");
	MN_DrTextA_CS(buf, off, y+=9);
	
	strcpy(buf, netDeathmatch? "DEATHMATCH" : "CO-OP");
	strcat(buf, netNomonsters? ", NO MONSTERS" : " WITH MONSTERS");
	MN_DrTextA_CS(buf, off, y+=9);

	if(netMobDamageModifier > 1)
	{
		sprintf(buf, "D:%i", netMobDamageModifier);
		MN_DrTextAYellow_CS(buf, 285, y2+=9);
	}
	if(netMobHealthModifier > 1)
	{
		sprintf(buf, "H:%i", netMobHealthModifier);
		MN_DrTextAYellow_CS(buf, 285, y2+=9);
	}
}
...
void SCGameSetupDamageMod(int option)
{
	if(option == RIGHT_DIR)
	{
		if(netMobDamageModifier < 100) netMobDamageModifier++;
	}
	else if(netMobDamageModifier > 1) netMobDamageModifier--;
}
...
void SCGameSetupHealthMod(int option)
{
	if(option == RIGHT_DIR)
	{
		if(netMobHealthModifier < 20) netMobHealthModifier++;
	}
	else if(netMobHealthModifier > 1) netMobHealthModifier--;
}
...
void SCChooseHost(int option)
{
	...
	if(JoinMenu.itemCount == 1) return; // Just "(searching...)"
