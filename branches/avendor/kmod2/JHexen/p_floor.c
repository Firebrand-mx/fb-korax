...
//	if(sector->ceilingpic == skyflatnum) P_SkyFix();
...
int EV_DoFloor(line_t *line, byte *args, floor_e floortype)
{
	...
	while ((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0)
	{
		...
		//
		rtn = 1;
		floor = gi.Z_Malloc (sizeof(*floor), PU_LEVSPEC, 0);
...
int EV_BuildPillar(line_t *line, byte *args, boolean crush)
{
	...
	while((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0)
	{
		...
		pillar = gi.Z_Malloc(sizeof(*pillar), PU_LEVSPEC, 0);
	...
	int timer)
{
	...
	while((sectorIndex = P_FindSectorFromTag(tag, sectorIndex)) >= 0)
	{
		...
		retCode = true;
		waggle = gi.Z_Malloc(sizeof(*waggle), PU_LEVSPEC, 0);
