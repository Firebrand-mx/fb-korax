...
void H2_PreInit(void)
{
	...
	if(gi.version < DOOMSDAY_VERSION) 
		gi.Error("Korax Gameplay Mod requires at least Doomsday "DOOMSDAY_VERSION_TEXT"!\n");//JHexen
...
//	ddfont_t	cfont;
	...
	gi.Message("--- Korax Gameplay Mod Init ---\n%s\n", VERSIONTEXT);//JHexen
...
__declspec(dllexport) game_export_t * GetGameAPI(game_import_t *imports)
{
